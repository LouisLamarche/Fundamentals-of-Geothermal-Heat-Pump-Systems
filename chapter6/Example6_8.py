...
while not ok:
    ...
    Lch1 = (qa_ch*Rsa)/(To - Tf_ch - Tpch)
    Lch2 = (qm_ch*Rsm)/(To - Tf_ch - Tpch)
    Lch3 = (qh_ch*Rsh)/(To - Tf_ch- Tpch)
    Lch4 = (qh_ch*Rb)/(To - Tf_ch - Tpch)
    L_ch = Lch1 + Lch2 + Lch3 + Lch4
    # calcul de la longeur Climatisation
    Lcl1 = (qa_cl*Rsa)/(To - Tf_cl - Tpcl)
    Lcl2 = (qm_cl*Rsm)/(To - Tf_cl - Tpcl)
    Lcl3 = (qh_cl*Rsh)/(To - Tf_cl- Tpcl)
    Lcl4 = (qh_cl*Rb)/(To - Tf_cl - Tpcl)
    L_cl = Lcl1 + Lcl2 + Lcl3 + Lcl4
    L = max(L_ch,L_cl)
    print ('The total length is '+ str(L) + ' m')
    H = L/nt
    print ('length per borehole is '+ str(H) + ' m')
    qp = qa/L
    print ('qp is '+ str(qp) + ' W/m')
    if Tp_choice == 'ASHRAE':
        Tpn = Tp_ashrae(nx,ny,b,qp,als,ks,n_years,H = H)  # alpha must be in  m2/day , k in W/mK
    else:
        Tpn = Tp_fls(nx,ny,b,qp,als,ks,n_years,H,zot)  # alpha must be in  m2/day , k in W/mK
    print ('Penalty temperature is '+ str(Tpn) + ' C')
    if (abs(L-Li)) < delta:
        ok = True
    else:
        compt = compt + 1
