...
# topologie du réseau
if npac == 6:
    mboucle = [np.array([1,7,13,14,15,16,17,18,-19]),np.array([2,8,-3,-13]),np.array([3,9,-4,-14]),np.array([4,10,-5,-15]),np.array([5,11,-6,-16]),np.array([6,12,-7,-17]),np.array([19,20])]
elif npac == 5:
    mboucle = [np.array([1,6,11,12,13,14,15,16,-17]),np.array([2,7,-3,-12]),np.array([3,8,-4,-13]),np.array([4,9,-5,-14]),np.array([5,10,-6,-15]),np.array([17,18])]
elif npac == 4:
    mboucle = [np.array([1,5,9,10,11,12,13,14,-15]),np.array([2,6,-3,-11]),np.array([3,7,-4,-12]),np.array([4,8,-5,-13]),np.array([15,16])]
elif npac ==3:
    mboucle = [np.array([1,4,7,8,9,10,11,12,-13]),np.array([2,5,-3,-10]),np.array([3,6,-4,-11]),np.array([13,14])]
elif npac == 2:
    mboucle = [np.array([1,3,5,6,7,8,9,10,-11]),np.array([2,4,-3,-9]),np.array([11,12])]
# valeurs initiales
debit1 = 0.0008
debit_nom = debit1*npac
qpipe[0] = debit_nom
if npac == 6:
    tube = [3,1.,1.,1.,1.,1.,1.,1.,1.25,1.5,2,2,2,2,1.5,1.25,1.,3,2,3]
elif npac == 5:
    tube = [3,1,1,1,1,1,1,1.25,1.5,2,2,2,2,1.5,1.25,3,3,3]
elif npac == 4:
    tube = [3,1,1,1,1,1,1.25,1.5,2,2,2,2,1.5,3,3,3]
elif npac == 3:
    tube = [3,1,1,1,1,1.25,1.5,2,2,2,2,3,3,3]
elif npac == 2:
    tube = [3,1,1,1,1.25,1.5,2,2,2,3,3,3]
Lpipe[0] = 0.0
...
from hpompe_md import *
Pompe[0] = pompe_2007()
Pompe[npipes-1] = pompe_2007()
#Pompe[npipes-1] = 0.6*g*pompe_2007()
if VFD:
    for i in range(0,4):
        Pompe[0][i] = Pompe[0][i]*faca**(i-1)
        Pompe[npipes-1][i] = Pompe[npipes-1][i]*facb**(i-1)
p =Pompe[0]
my_pipes = []
for i in range(0,npipes):
...
mp1 = rho*qpipes[0]
W_pompe1 = mp1*ghp[0]
mp2 = rho*qpipes[npipes-1]
W_pompe2 = mp2*ghp[npipes-1]
deb1 = qpipes[0]*1000 # debit pompe  l/s
deb2 = qpipes[npipes-1]*1000 # debit pompe  l/s
p_rend1 =  np.array([ 0.03587946, -2.50350394, 22.73341582, 21.44849976])
p_rend2 =  np.array([ 0.03587946, -2.50350394, 22.73341582, 21.44849976])
rend1 = np.polyval(p_rend1,deb1/faca)/100 # efficiency
rend2 = np.polyval(p_rend2,deb2/faca)/100 # efficiency
P_elec = W_pompe1/rend1 + W_pompe2/rend2
print ('Fluid power = ' + str(W_pompe1+W_pompe2) + ' Watss')
...
if ecri:
    if npac == 6 and VFD == 0:
        f = open("U:/documents/notes_ang/fig/chapter8/pac6m.txt", "w")
    elif npac == 6 and VFD == 1:
        f = open("U:/documents/notes_ang/fig/chapter8/pac6bm_5.txt", "w")
    if npac ==5 and VFD == 0:
        f = open("U:/documents/notes_ang/fig/chapter8/pac5m.txt", "w")
    if npac ==5 and VFD == 1:
        f = open("U:/documents/notes_ang/fig/chapter8/pac5bm.txt", "w")
    elif npac == 4 and VFD == 0:
        f = open("U:/documents/notes_ang/fig/chapter8/pac4m.txt", "w")
    elif npac == 4 and VFD == 1:
        f = open("U:/documents/notes_ang/fig/chapter8/pac4bm.txt", "w")
    elif npac == 3 and VFD == 0:
        f = open("U:/documents/notes_ang/fig/chapter8/pac3m.txt", "w")
    elif npac == 3 and VFD == 1:
        f = open("U:/documents/notes_ang/fig/chapter8/pac3bm.txt", "w")
    elif npac == 2 and VFD == 0:
        f = open("U:/documents/notes_ang/fig/chapter8/pac2m.txt", "w")
    elif npac == 2 and VFD == 1:
        f = open("U:/documents/notes_ang/fig/chapter8/pac2bm.txt", "w")
    ss = str(deb) + "," + str(h_tot)  + ","+  str(P_elec) +  "\n"
...
##gpmo = gpm_ls(Qlo)
##h_loo = ft_m(ghp[19]/g)
##print ('h loop = ' + str(h_loo) + ' ft')
##print ('gpm lop = ' + str(gpmo) + ' gpm')
##nb = 6
##qloop = Ql/nb/1000
...
print ('Fluid power = ' + '%.2f' % (W_pompe1 + W_pompe2)  + ' W')
